# train_kmm_autonomous_no_obs_slice_dec.py
# x_{t+1} ≈ (A g(x_t) + G(g(x_t)))[:,:d], with A = I + A_param (learnable)
...
def train(args):
    ...
    # save
    os.makedirs(args.outdir, exist_ok=True)
    ckpt_path = os.path.join(args.outdir, "kmm_autonomous_no_obs_slice_dec.pt")
    A_effective = model.get_A().detach().cpu().numpy()
    torch.save({
        "state_dict": model.state_dict(),     # 包含 A_param
    ...
    plt.xlabel("time"); plt.ylabel("state value"); plt.title("KMM (slice decoder) rollout")
    ...
    if d == 3:
        ...
        ax.set_title("3D rollout (KMM, slice decoder)")
...
if __name__ == "__main__":
    ...
    # misc
    ap.add_argument("--outdir", type=str, default="kmm_outputs_no_obs_slice_dec")
