...
def compute_A(Y, X, rcond=1e-8):
    A, *_ = np.linalg.lstsq(X.T, Y.T, rcond=rcond)
    return A.T
...
def plot_forecast_time(truth, preds, dt, outpath, start_index, a_param=None):
    ...
    ttl = "Forecast on a test trajectory (Hankel-DMD)"
...
def main():
    ...
    ap.add_argument("--Ld", type=int, default=20, help="Number of delays (time-delay length)")
    ap.add_argument("--train_ratio", type=float, default=0.7, help="Fraction of trajectories for training")
    ...
    ap.add_argument("--outdir", type=str, default="hdmd_outputs", help="Directory to save outputs")
    ...
    # Save artifacts
    np.savez_compressed(
        os.path.join(args.outdir, "learned_A_and_split.npz"),
