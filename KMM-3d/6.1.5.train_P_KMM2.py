# train_kmm_autonomous_no_obs_slice_dec_idlift.py
# x_{t+1} ≈ (A g(x_t) + G(g(x_t)))[:,:d], with:
#   - Identity Lift: g(x) = concat[x, h(x)]  (first d dims = x, hard constraint)
#   - A = I + A_param (learnable)
#   - Decoder = slice first d coords
...
class GIdentityLift(nn.Module):
    """Hard identity lift: g(x) = concat[x, extra(x)]"""
...
class KMM(nn.Module):
    ...
    def state_step(self, x):
        ...
        return z1_hat[:, :self.d]             # slice decoder
...
def train(args):
    ...
    # save
    os.makedirs(args.outdir, exist_ok=True)
    ckpt_path = os.path.join(args.outdir, "kmm_autonomous_no_obs_slice_dec_idlift.pt")
    A_effective = model.get_A().detach().cpu().numpy()
    torch.save({
        "state_dict": model.state_dict(),     # 包含 A_param 与 g/G 权重
        "A_effective": A_effective,           # 直接存有效 A，方便后续画谱
        "args": vars(args),
        "mean": mean.numpy(),
        "std": std.numpy(),
        "dt": dt
    ...
    plt.xlabel("time"); plt.ylabel("state value"); plt.title("KMM (identity-lift, slice decoder) rollout")
    ...
    if d == 3:
        ...
        ax.set_title("3D rollout (KMM, identity-lift)")
...
if __name__ == "__main__":
    ...
    # dims
    ap.add_argument("--n", type=int, default=8, help="observable dim; must be >= d")
    ...
    # misc
    ap.add_argument("--outdir", type=str, default="kmm_outputs_no_obs_slice_dec_idlift")
